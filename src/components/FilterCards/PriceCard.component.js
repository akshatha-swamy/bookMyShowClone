            ...
            <div className="w-full bg-white rounded-md overflow-hidden shadow-lg p-3">
                <div className="flex justify-between cursor-pointer">
                    <h1 className="text-xl font-semibold flex items-center text-buttonPink-300">
                        {isDropdownOpen ? <BiChevronUp style={{color:"#4b5563"}}  onClick={toggleDropdown}/> : <BiChevronDown style={{color:"#4b5563"}} onClick={toggleDropdown}/>}Price
                    </h1>
                    <p className="text-gray-600"  onClick={toggleDropdown}>clear</p>
                </div>
                {isDropdownOpen && (
                    <div className="flex flex-wrap m-2 max-h-40 ">
                        <FilterButton filter='Free' />
                        <FilterButton filter='0-500' />
                        <FilterButton filter='501-2000' />
                        <FilterButton filter='Above 2000' />
